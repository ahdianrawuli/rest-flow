        ...
        try {
            ...
            for (let m of monitors) {
                try {
                    ...
                    console.log(`[CRON] Invalid cron ignored for monitor ${m.id}: ${m.schedule_cron}`);
                }
            }
...
async function connectDB() {
    ...
    try {
        ...
        await pool.query(`CREATE TABLE IF NOT EXISTS functions (id INT AUTO_INCREMENT PRIMARY KEY, workspace_id INT NOT NULL, name VARCHAR(255) NOT NULL, script LONGTEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE, UNIQUE KEY (workspace_id, name))`).catch(()=>Object());
...
async function injectExampleWorkspace(userId, workspaceId) {
    try {
        ...
        const fnScript = `// This function generates a random transaction ID
const timestamp = Date.now();
const randomStr = Math.random().toString(36).substring(2, 8).toUpperCase();
return 'TRX-' + timestamp + '-' + randomStr;`;
        await pool.query('INSERT INTO functions (workspace_id, name, script) VALUES (?, ?, ?)', [workspaceId, 'generateTransactionId', fnScript]);
...
app.get('/api/workspaces/:workspaceId/functions', authenticateToken, async (req, res) => {
    try { const [rows] = await pool.query('SELECT * FROM functions WHERE workspace_id = ? ORDER BY name ASC', [req.params.workspaceId]); res.json(rows); } catch (e) { res.status(500).json({ error: 'Error' }); }
});
app.post('/api/workspaces/:workspaceId/functions', authenticateToken, async (req, res) => {
    try { const [r] = await pool.query('INSERT INTO functions (workspace_id, name, script) VALUES (?, ?, ?)', [req.params.workspaceId, req.body.name, req.body.script]); res.status(201).json({ id: r.insertId }); } catch (e) { res.status(500).json({ error: 'Error' }); }
});
app.put('/api/workspaces/:workspaceId/functions/:id', authenticateToken, async (req, res) => {
    try { await pool.query('UPDATE functions SET name=?, script=? WHERE id=? AND workspace_id=?', [req.body.name, req.body.script, req.params.id, req.params.workspaceId]); res.json({ message: 'Updated' }); } catch (e) { res.status(500).json({ error: 'Error' }); }
});
app.delete('/api/workspaces/:workspaceId/functions/:id', authenticateToken, async (req, res) => {
    try { await pool.query('DELETE FROM functions WHERE id=? AND workspace_id=?', [req.params.id, req.params.workspaceId]); res.json({ message: 'Deleted' }); } catch (e) { res.status(500).json({ error: 'Error' }); }
});

const applyVariablesAsyncBackend = async (text, varsArr, funcsArr) => {
    ...
    (varsArr || []).forEach(v => {
        if (v.var_key && v.var_value !== undefined && v.var_value !== null) {
            ...
            result = result.replace(regex, typeof v.var_value === 'object' ? JSON.stringify(v.var_value) : String(v.var_value));
        }
    });
    ...
    for (const m of matches) {
        ...
        if (fnDef) {
            try {
                ...
                result = result.replace(m.full, valStr);
            } catch (e) {
                console.error(`Error executing function ${m.fnName} in mock:`, e);
            }
        }
    }
...
app.all('/api/mock/:workspaceId/*', async (req, res) => {
    try {
        ...
        const mock = rows[0]; 
        const [varsList] = await pool.query('SELECT var_key, var_value FROM variables WHERE workspace_id = ?', [req.params.workspaceId]);
        const [funcsList] = await pool.query('SELECT name, script FROM functions WHERE workspace_id = ?', [req.params.workspaceId]);

        if (mock.headers) { 
            let h = []; 
            try { h = typeof mock.headers === 'string' ? JSON.parse(mock.headers) : mock.headers; } catch(e){} 
            for (const hi of h) { 
                if (hi.key && hi.value) {
                    const key = await applyVariablesAsyncBackend(hi.key, varsList, funcsList);
                    const val = await applyVariablesAsyncBackend(hi.value, varsList, funcsList);
                    res.setHeader(key, val);
                }
            } 
        }
        
        let body = await applyVariablesAsyncBackend(mock.body || '', varsList, funcsList); 
        if (!res.getHeader('content-type') && body.trim().startsWith('{')) res.setHeader('Content-Type', 'application/json');
